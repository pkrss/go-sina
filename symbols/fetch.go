...
type Exchange2Items struct {
	...
	ItemTypes    []ExchangeItemsType `json:"itemTypes"`
...
func fetchExchangeSymbols(foundExchange *Exchange2Items) (retList []map[string]string, retE error) {
	...
	case ExchangeType_Futures:
		return fetchExchangeSymbols_Futures(foundExchange)
	}
	return
}

func fetchExchangeSymbols_Futures(foundExchange *Exchange2Items) (retList []map[string]string, retE error) {
	...
	for _, itemTypes := range foundExchange.ItemTypes {
		l, e := fetchExchangeSymbols_FuturesType(foundExchange, &itemTypes)
		if e != nil {
			retE = e
			return
		}
...
func fetchExchangeSymbols_FuturesType(foundExchange *Exchange2Items, exchangeItemsType *ExchangeItemsType, params ...string) (retList []map[string]string, retE error) {
